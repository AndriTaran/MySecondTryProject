...
async function downloadProfile(ctx, dstPath, filename, sourcemapPath, raw, shouldGenerateSourcemap, port) {
  try {
    ...
    //11111111111111 execSyncWithLog(`adb shell run-as ${packageName} cp cache/${file} /sdcard`); // If --raw, pull the hermes profile to dstPath

    if (raw) {
      //1111111111111111 execSyncWithLog(`adb pull /sdcard/${file} ${dstPath}`);
      execSyncWithLog(
      ...
      _cliTools().logger.success(`Successfully pulled the file to ${dstPath}/${file}`);
        ...
        // 1111111111111 execSyncWithLog(`adb pull /sdcard/${file} ${tempFilePath}`); // If path to source map is not given
      execSyncWithLog(
